...
### Latest Updates
- **Buyer Dashboard Navigation Refinement**: Removed duplicate navigation cards (My Orders, Wishlist, Profile) from buyer dashboard main page since they already exist in the sidebar, eliminating UI redundancy. Fixed recent orders section to intelligently redirect based on payment status - unpaid orders (pending, payment_pending, payment_failed) now redirect to `/payment/:orderId` with "Click to pay" indicator, while paid orders redirect to order tracking. Updated DashboardLayout navigation to properly handle buyer-specific global routes (/notifications, /settings) while preserving role-specific routes for other roles (/seller/notifications, /rider/settings, etc.), preventing navigation regressions across all user types.
- **Role-Based Dashboard Separation**: Implemented complete separation between admin and super_admin dashboards. Created AdminDashboardRouter component that checks user role and routes to appropriate dashboard - super_admins get full AdminDashboardConnected with ALL features (Branding, Settings, Users, Applications, etc.), while regular admins get limited AdminDashboard with restricted access (no Branding, Settings, Users, or Applications). Updated DashboardSidebar to have separate menu items for each role. Added proper auth redirect in router to send unauthorized users to /auth page.
- **Super Admin API Permissions**: Fixed all user management API endpoints (8 total: GET/POST/PATCH/DELETE /api/users, /api/users/:id, /api/users/:id/approve, /api/users/:id/reject, /api/users/:id/status) to allow both "admin" and "super_admin" roles instead of only "admin". This resolves 403 errors that super_admin was receiving when accessing admin dashboard features.
- **Admin Dashboard Error Fix**: Fixed buyers.map TypeError in AdminDashboardConnected by adding proper error handling to buyers query - checks response.ok status, validates data is an array before returning, and uses Array.isArray() guard when creating buyerMap. Dashboard now gracefully handles API errors without crashing.
- **Profile Update Enhancement**: Added 'name' field to allowed fields in /api/profile PATCH endpoint alongside existing username, phone, address, city, and country fields. Users can now successfully update their profile names without errors.
- **Category Grid Size Reduction**: Reduced category grid max-width from ~982px to ~662px by changing `grid-template-columns` from `minmax(280px, 1fr)` to `minmax(200px, min(33.333%, 320px))` for .category-grid and `minmax(180px, min(33.333%, 280px))` for .category-grid-single-store in index.css. Categories no longer cover entire page width and maintain appropriate sizing across all breakpoints.
- **Admin Footer Pages Manager**: Created comprehensive footer pages management system with AdminFooterPagesManager component at `/admin/footer-pages`. Implemented full CRUD operations for managing Returns & Refunds and all footer page visibility controls. Added storage methods (getAllFooterPages, createFooterPage, updateFooterPage, deleteFooterPage) to DbStorage and IStorage interface. Created secured API endpoints (GET/POST/PATCH/DELETE /api/admin/footer-pages) with Zod validation using insertFooterPageSchema and role-based access control for admin and super_admin. Component includes complete data-testid coverage on all interactive and display elements for automated testing support.
- **Profile Avatar Fallback Fix**: Fixed Profile.tsx avatar fallback to use `profile?.name` instead of non-existent `profile?.username` property, resolving LSP error and ensuring proper display of user initials when profile image is unavailable.
- **Delivery Fee System Overhaul**: Completely removed all default/hardcoded delivery fees across the platform. Checkout now uses ONLY zone-based pricing from delivery zones table. Bus delivery uses 50% of zone fee, rider delivery uses full zone fee, pickup is free. Removed hardcoded fallback values (previously 5 for bus, 10 for rider) from both CheckoutConnected.tsx and Checkout.tsx UI labels.
- **Role-Based Login Redirect**: Fixed AuthPage to redirect users to their role-specific dashboards after login instead of always redirecting to home page. Super_admin and admin redirect to /admin, seller to /seller, rider to /rider, buyer to /buyer, agent to /agent. This ensures all users land on their appropriate dashboards immediately after authentication.
